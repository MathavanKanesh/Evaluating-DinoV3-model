# Faster preprocessing: Load 20 images and masks, preprocess each one, extract DinoV3 features, feed into segmentation head, calculate loss 
# Backpropagate, update segmentation head weights, repeat for multiple epochs,
# then evaluate the trained model using Dice Score
...
optimizer = torch.optim.SGD(  #Updates segmentation head weights 
...
for epoch in range(5): # an epoch is one complete pass through the entire training datase
    ...
    for i in range(len(image_files)):
        ...
        # Load image
        image = Image.open(image_files[i]).convert("RGB") # Convert all image files to RGB
        ...
        # Load mask
        mask = Image.open(masks_files[i]).convert("L") # Convert mask files to "Luminance" (Grayscale)
        mask = mask.resize((683, 384)) # Resize mask 
    ...
    average_loss = total_loss / len(image_files) # Average loss across all images 

    print("Epoch:", epoch + 1, "Average loss:", average_loss)

#Epoch: 1 Average loss: 0.5470159307122231
#Epoch: 2 Average loss: 0.37542294561862943
#Epoch: 3 Average loss: 0.3306753054261208
#Epoch: 4 Average loss: 0.30009583085775376
#Epoch: 5 Average loss: 0.27603254094719887
# --> Loss is decreasing with each training loop 
...
#Evaluation Loop: Dont update weights (no loss.backward, optimizer.step because evaluating now, not training anymore)
...
for i in range(len(image_files)):
    ...
    mask = Image.open(masks_files[i]).convert("L")

    mask = mask.resize((683, 384))
...
average_dice_score = total_dice_score / len(image_files)

print("Average Dice Score:", average_dice_score)
# Average Dice Score: 0.508769...
...
# Training on all 20 images makes it so that the model doesn't generalize well on new data 